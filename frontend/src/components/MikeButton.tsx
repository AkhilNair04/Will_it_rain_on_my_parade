          ...
          <div className="bg-white/5 p-4 rounded-md shadow-sm text-left">
            <div className="flex items-start justify-between">
              <div>
                <h3 className="text-white font-semibold">
                  {backendResponse.input_location?.name || backendResponse.city || "Location"}
                </h3>
                <p className="text-sm text-gray-300">
                  {backendResponse.input_location?.forecast_date || backendResponse.date || ""}
                </p>
              </div>
              <div className="text-right">
                <p className="text-lg text-white font-bold">
                  {backendResponse.prediction_output?.predicted_rainfall_mm ??
                    backendResponse.predicted_rainfall_mm ??
                    "-"}{" "}
                  mm
                </p>
                <p className="text-sm text-gray-300">
                  {backendResponse.prediction_output?.rain_outlook ?? backendResponse.rain_outlook ?? ""}
                </p>
              </div>
            </div>
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 text-sm text-gray-300">
              <div>
                ...
                <div>
                  {backendResponse.prediction_output?.temperature_outlook ??
                    backendResponse.temperature_outlook ??
                    "-"}
                </div>
              </div>
              <div>
                ...
                <div>
                  {backendResponse.prediction_output?.wind_outlook ?? backendResponse.wind_outlook ?? "-"}
                </div>
              </div>
              <div>
                <div className="font-medium text-white">Erosion Risk</div>
                <div>
                  {backendResponse.prediction_output?.erosion_risk ?? backendResponse.erosion_risk ?? "-"}
                </div>
              </div>
              <div>
                <div className="font-medium text-white">POP</div>
                <div>
                  {backendResponse.live_forecast_values?.api_pop_percent ?? backendResponse.api_pop_percent ?? "-"}%
                </div>
              </div>
            </div>
